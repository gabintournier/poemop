...
$title = 'Mes groupements terminés';
$title_page = 'Mes groupements terminés';
...
$res = getListeGroupementsTermines($co_pmp);
...
<?php
				...
				{
					if($regroupement["statut"] == 33) { $statut = "Non Facturé"; }
					if($regroupement["statut"] == 37) { $statut = "Facturé"; }
					if($regroupement["statut"] == 40) { $statut = "Payé"; }
?>
					<tr class="select termines">
						<input type="hidden" name="n_grp" value="<?= $regroupement["id"]; ?>">
						<td><?= $regroupement["id"]; ?></td>
						<td><?= $regroupement["numfact"]; ?></td>
