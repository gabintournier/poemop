...
if (!empty($_POST["charger_commandes"]))
{
	...
	{
		$res_cmd = getCommandesFournisseurZonePlusCmd($co_pmp, $_SESSION["zone_fournisseur"], $_SESSION["etat_1"], $_SESSION["etat_2"]);
	}
	else
	{
		$res_cmd = getFiltreCommandesActifs($co_pmp);
...
elseif (!empty($_SESSION["fournisseurs"]) && !empty($_SESSION["zone_fournisseur"]))
{
	$res_cmd = getCommandesFournisseurZonePlusCmd($co_pmp, $_SESSION["zone_fournisseur"], $_SESSION["etat_1"], $_SESSION["etat_1"]);
	if(!empty($_POST["exporter_cmd"]))
	{
		exporterListeCommandes($co_pmp, $res_cmd);
		$res_cmd = getCommandesFournisseurZonePlusCmd($co_pmp, $_SESSION["zone_fournisseur"], $_SESSION["etat_1"], $_SESSION["etat_1"]);
...
if(!empty($_POST["ajouter_liste_groupement"]))
{
	...
	$nb_ok = 0;
	$id_grp = $_GET["id_grp"];
	$id = isset($_POST["ids_cmd"]) ? trim($_POST["ids_cmd"], ";") : "";
	$id_cmd = $id !== "" ? array_filter(explode(";", $id), 'strlen') : [];
	// Sécurise la récupération des commandes visibles si ids_cmd est vide
	if (empty($id_cmd) && !empty($_POST["id_cmde"]) && is_array($_POST["id_cmde"])) {
		$id_cmd = array_filter(array_map('trim', $_POST["id_cmde"]), 'strlen');
	}
	if(empty($id_cmd))
	{
		$message_type = "error";
		$message_icone = "fa-times";
		$message_titre = "Aucune commande";
		$message_mod = "Aucune commande sélectionnée pour le basculement.";
	}
	foreach ($id_cmd as $cmd_id) { // on renomme pour éviter de réécrire $id_cmd
	    $cmd = getCommandeDetailsClientsActifs($co_pmp, $cmd_id);
		$old_grp = isset($cmd["groupe_cmd"]) ? $cmd["groupe_cmd"] : '';
		// Si la commande n'est pas groupée ou déjà dans le groupement cible, on bascule directement
		if ($old_grp == 0 || $old_grp == $id_grp) {
			...
			{
				$nb_ok++;
				TraceHisto($co_pmp, $cmd_id, 'Basculer Groupement', $old_grp . " -> " . $id_grp);
			}
		}
		// Sinon on prépare la modal pour décider
		else
		{
			$id_ko .= $cmd_id . ";";
		}
	...
	if($nb_ok > 0)
	{
		...
		$message_mod = "Les commandes sélectionnées ont été ajoutées au groupement.";
	}
	if($nb_ok == 0 && $id_ko === "" && !empty($id_cmd))
	{
		$message_type = "error";
		$message_icone = "fa-times";
		$message_titre = "Aucune commande basculée";
		$message_mod = "Ces commandes ne peuvent pas être rattachées de cette façon à un groupement, vérifiez les commandes.";
	}

}

if(!empty($_POST["basculer_commande"]) || isset($_POST["tous_basculer"]))
{
	$id_grp = $_GET["id_grp"];
	$tous = isset($_POST["tous_basculer"]) ? "1" : "0";
	$nb_ok = 0;
	$cmde_ids_from_post = [];
	foreach ($_POST as $k => $v) {
		if (strpos($k, 'cmde_id_grp_') === 0) {
			$cmde_ids_from_post[] = (int)$v;
		}
	}
	if($tous == 1)
	{
		$id = isset($_POST["id_basculer"]) ? trim($_POST["id_basculer"], "; \n\r\t") : "";
		$id_cmds = $id !== "" ? array_filter(array_map('trim', explode(";",$id)), 'strlen') : [];
		// On combine id_basculer et les valeurs cmde_id_grp_* pour être sûr de ne rien perdre
		$targets = array_unique(array_filter(array_map('intval', array_merge($id_cmds, $cmde_ids_from_post)), function($v){ return $v > 0; }));
		foreach ($targets as $id_cmd)
		{
			...
			if($res)
			{
				TraceHisto($co_pmp, $id_cmd, 'Basculer Groupement', $id_grp);
				$nb_ok++;
			}
	...
	{
		$nb = isset($_POST['nb_cmd_basculer']) ? (int)$_POST['nb_cmd_basculer'] : count($cmde_ids_from_post);
		for ($i=0; $i < $nb; $i++)
		{
			$id = 'cmde_id_grp_' . $i;
			if (isset($_POST[$id]))
	...
	if ($nb_ok > 0) {
		...
		$message_icone = "fa-times";
		$message_titre = "Aucune commande basculée";
		$message_mod = "Aucune commande n’a été déplacée. Vérifiez la sélection ou le statut des commandes.";
	}
}
...
<?php if(isset($message_type) && isset($message_mod)) { ?>
<div class="toast <?= $message_type; ?>" style="margin: 10px 0 15px;">
	<div class="message-icon  <?= $message_type; ?>-icon">
		<i class="fas <?= isset($message_icone) ? $message_icone : 'fa-info'; ?>"></i>
	</div>
	<div class="message-content ">
		<div class="message-type">
			<?= isset($message_titre) ? $message_titre : ''; ?>
		</div>
		<div class="message">
			<?= $message_mod; ?>
		</div>
...
?>
<form method="post">
<div class="modal fade" id="AjouterCommande" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered" style="max-width: 85%;">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<hr>
				<div class="tableau" style="height:520px;">
...
<?php
						$i = 0;
						$id_b = "";
						$ids_cmds = explode(";",$id_ko);
						foreach ($ids_cmds as $id_cmd)
						{
							$cmd = getCommandeDetailsClientsActifs($co_pmp, $id_cmd);
							$id_b .= $id_cmd . ";";
							if(isset($cmd))
							{
?>
...
?>
					...
					<input type="hidden" name="id_basculer" value="<?php print $id_b; ?>">
				</div>
			</div>
			...
			<div class="col-sm-3" style="margin-left: -1%;">
				...
				<div class="form-inline" style="margin: 2% 0 0 0;">
					...
					<div class="col-sm-7" style="padding:0">
						<select class="form-control" name="fournisseur_ajax" style="width:100%;">
...
?>
			...
			<div class="col-sm-2 align-self-center" style="margin-left:-3%;margin-top: 33px;">
				<div class="btn-form">
			...
			<div class="col-sm-2 align-self-center" style="border-left: 1px solid #0b242436;margin-left:-3%;">
				<input type="submit" name="ajouter_liste_groupement" value="AJOUTER LISTE GROUPEMENT" class="btn btn-primary"  style="width: 132%;margin-top:2%;">
				<div class="form-inline" style="margin: 2% 0 2% 0;">
...
?>
<script src="/admin/js/script_regroupements.js" charset="utf-8"></script>
<script>
