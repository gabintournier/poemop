...
function getCommandesGroupements(&$co_pmp)
{
...
function getCommandesGroupementsVol(&$co_pmp, $type)
{
...
function getGroupementPossibleVol(&$co_pmp, $type)
{
	$query = "  SELECT SUM(pmp_commande.cmd_qte) AS vol, COUNT(pmp_utilisateur.user_id)
                FROM pmp_commande, pmp_utilisateur, jjj_users, pmp_zone_cp, pmp_regrp_zone, pmp_regroupement
                WHERE pmp_commande.user_id = pmp_utilisateur.user_id
                AND pmp_utilisateur.user_id = jjj_users.id
                AND pmp_utilisateur.code_postal_id = pmp_zone_cp.code_postal_id
...
function getPlusieursGroupementsPossiblesVol(&$co_pmp, $type)
{
	$query = "  SELECT SUM(pmp_commande.cmd_qte) AS vol
                FROM pmp_commande, pmp_utilisateur, jjj_users, pmp_zone_cp, pmp_regrp_zone, pmp_regroupement
                WHERE pmp_commande.user_id = pmp_utilisateur.user_id
...
if(!empty($_POST["affecter_commandes"]))
{
		...
		for($i=0;$i<$_POST['nb_cmd'];$i++)
		{
			...
			if($cmd == 1)
			{
				...
				if(isset($id_grp))
				{
					if($id_grp > 0)
					{
						$res_refus = getCommandeGroupementRefus($co_pmp, $user_id, $id_grp);
						...
						if($num_refus > 0)
						{
							...
							$message = "Certaines commandes n'ont pas pu être affectées suite à un refus de tarif.";
						}
						else
						{
