...
<?php
			...
			$id_contact = explode(";", $select_contact["mail_to"]);
			while ($contact = mysqli_fetch_array($res))
			{

				$value = compare($contact["id"], $id_contact);
?>
			<tr class="select contact">
				<input type="hidden" name="contact_id[]" value="<?= $contact["id"]; ?>">
				<td style="padding: 0.6rem;">
					<input type="checkbox" name="select_contact_<?php print $i++; ?>" id="select_contact" class="switch value check" <?php if(isset($value)) { if($value == $contact["id"]) { echo "value='1'"; } } ?>>
				</td>
